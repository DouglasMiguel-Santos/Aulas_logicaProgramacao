/*
 11 - Faça um algoritmo que leia o valor de um produto e determine o valor que deve ser pago, 
 conforme a escolha da forma de pagamento
 pelo comprador e imprima na tela o valor final do produto a ser pago.
  Utilize os códigos da tabela de condições de pagamento para efetuar o cálculo adequado. 
  
 Tabela de Código de Condições de Pagamento

 

 1 - À Vista em Dinheiro ou Pix, recebe 15% de desconto

 2 - À Vista no cartão de crédito, recebe 10% de desconto

 3 - Parcelado no cartão em duas vezes, preço normal do produto sem juros

 4 - Parcelado no cartão em três vezes ou mais, preço normal do produto mais juros de 10% 
 */

let produtos = {
  geladeira: 2000.98,
  fogao: 600,
  mesa: 300,
  sofa: 1555.99,
};

function pagamento(produto, formadepagamento) {
  if (formadepagamento == 1) {
    return produto - [(15 / 100) * produto];
  }
  if (formadepagamento == 2) {
    return produto - [(10 / 100) * produto];
  }
  if (formadepagamento == 3) {
    return produto;
  }
  if (formadepagamento == 4) {
    let juros = (10 / 100) * produto;
    return produto + juros;
  }
}
console.log(
  " Vai ser pago o valor total de: ",
  pagamento(produtos.geladeira, 4)
);
